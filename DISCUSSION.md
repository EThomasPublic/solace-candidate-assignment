...
1. Implement a client side filter. I had initially built this with a client side filter but decided to try to filter using drizzle ORM since I had not used it.
2. More Styling. I spend most of my cleaning up the code and breaking things out into smaller, consumable chunks.  I toyed around with tailwind a bit to clean up the UI (since I'd never used Tailwind before) but did the bare minimum. 
